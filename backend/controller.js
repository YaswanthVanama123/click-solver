...
const storeNotification = async (req, res) => {
  ...
  try {
    const result = await client.query(
      ...
      [title, body, JSON.stringify(data), receivedAt, workerId, userNotificationId, fcmToken]
    );
    res.status(200).json(result.rows[0]);
  } catch (err) {
    console.error('Error storing notification:', err);
        ...
        if (updateResult.rowCount === 0) {
          ...
          if (fcmTokens.length > 0) {
            ...
            const multicastMessage = {
              ...
              data: {
                user_notification_id: user_notification_id.toString(),
              },
            };
...
const serviceCompleted = async (req, res) => {
  ...
  if (status === "cancel") {
    ...
    // Update end_time if it's not set
    const updateQuery = `
      UPDATE servicecall
      SET end_time = $1
      WHERE notification_id = $2
    `;
    const updateResult = await client.query(updateQuery, [end_time, notification_id]);
  ...
  try {
      ...
      if (fcmTokens.length > 0) {
        ...
        const multicastMessage = {
          ...
          data: {
            user_notification_id: decodedId.toString(),
          },
        };
...
const getWorkDetails = async (req, res) => {
  ...
  try {
    const queryText = `
      SELECT city, pincode, area, alternate_phone_number, alternate_name, service
